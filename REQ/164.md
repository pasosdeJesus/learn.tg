...
> **Proposed engine:** identity (future `pq-wallet` / `identity` engine,
> https://github.com/pasosdeJesus/learn.tg/issues/166/176/177) — **independent of gdcluster**. The reputation of
> churches/pastors does **not** belong to the GD domain.
>
> **Privacy-first (2026-08-29):** in a restricted region (region type 2) the
> reputation (or identity) of churches/pastors cannot be publicly exposed.
> Design requirements:
> - **Mandatory pseudonymity**: reputation under an **alias**, not linked to the
>   real person/church.
> - **Aggregation**: data per cluster/country, not per person.
> - **Minimal data** and the ability to change alias/name.
> - Synergy with #184 (serverless wallet) so the alias ↔ real identity link is
>   not centralized.
>
> **Note (2026-08-29):** how/where to implement it is undecided — define the scope
> together with the identity engine (166/176/177); do not implement it in gdcluster.
...
- R-#154 (Cluster and Country Ranking)
...
```
## Church Ranking
| Position | Church | Reputation | Level | Country |
|----------|---------|------------|-------|------|
| 1 | Peace Center | 85 | 🥈 Silver | 🇸🇱 |
| 2 | Esperanza Viva | 72 | 🥈 Silver | 🇸🇱 |
```

### 5.3 Blocked Display
```
## Church of the Offending Pastor
**Reputation:** -50 (⚠️ Penalized)
**Pastor:** Bloqueado
**Status:** Receives no funds from the churches fund
```
...
**Created:** 2026-06-29
**Status:** Pendiente
**Revisión 2026-09-25 (código):** ninguna migración ni tabla introduce campos de
reputación (`reputation`/`reputacion` no aparecen en `db/migrations`), así que el sistema de
reputación de iglesias y pastores sigue sin empezar. **No cerrable.**
