---
title: "R-#164: R-#164 Church and Pastor Reputation System"
labels: enhancement
---
...
|--------|---------|
| 1 month inactive | -2 |
| 3 months inactive | -10 |
| 6 months inactive | -30 |
| 12 months inactive | Reset to 0 |
...
```sql
CREATE TABLE pastor_reputation (
...
```sql
CREATE TABLE church_reputation (
    id SERIAL PRIMARY KEY,
...
```typescript
function calculateChurchReputation(churchId: number): number {
    // Positive reputation from activity
    const activityScore = getActivityScore(churchId);
    const referralBonus = getReferralBonus(churchId);
    const clusterBonus = getClusterBonus(churchId);
    const inactivityPenalty = getInactivityPenalty(churchId);
    
    // Negative reputation from pastor's status
    const pastor = getPastor(churchId);
    let pastorPenalty = 0;
    if (pastor && pastor.is_blocked) {
        pastorPenalty = 50; // Church gets -50 from blocked pastor
    }
    
    return activityScore + referralBonus + clusterBonus - inactivityPenalty - pastorPenalty;
}
```
...
```typescript
function calculatePastorReputation(pastorId: number): number {
    // Positive reputation
    const activityScore = getPastorActivityScore(pastorId);
    const referralBonus = getPastorReferralBonus(pastorId);
    const leadershipBonus = getPastorLeadershipBonus(pastorId);
    const inactivityPenalty = getPastorInactivityPenalty(pastorId);
    
    // Negative reputation (if blocked)
    const isBlocked = getPastorBlockedStatus(pastorId);
    if (isBlocked) {
        return -100;
    }
    
    return activityScore + referralBonus + leadershipBonus - inactivityPenalty;
}
```
...
|----------|--------|-------------|
| `/api/reputation/pastor/:id` | GET | Get pastor reputation |
| `/api/reputation/church/:id` | GET | Get church reputation |
| `/api/reputation/pastor/:id/block` | POST | Block a pastor |
| `/api/reputation/pastor/:id/unblock` | POST | Unblock a pastor |
| `/api/reputation/pastor/:id/appeal` | POST | Submit appeal |
| `/api/reputation/history/:type/:id` | GET | Get reputation history |
