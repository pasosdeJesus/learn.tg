...
| **pdJ's public key** | pdJ / learn.tg | `NEXT_PUBLIC_ADDRESS` — the learn.tg backend wallet address |
| **pdJ's private key** | pdJ / learn.tg | `PRIVATE_KEY` — the learn.tg backend wallet private key |
...
flowchart TB
    ...
    subgraph Encryption["Encryption Process (Server-Side)"]
        ...
        E[Encrypt DEK with pdJ's public key<br/>ECIES using NEXT_PUBLIC_ADDRESS]
    end
...
| **4** | Backend encrypts DEK with pdJ's public key via ECIES (using `NEXT_PUBLIC_ADDRESS`) |
...
| **pdJ's public key** | `NEXT_PUBLIC_ADDRESS` — the learn.tg backend wallet |
