...
| Registration number | Church registration | |
...
| Region preference | User profile | Preference |
...
The secp256k1-based components (ECDSA for user DEK derivation, ECIES for pdJ DEK encryption) are not post-quantum safe. A cryptographically relevant quantum computer would break ECC, exposing the DEK. Mitigation: R-#166 introduces SPHINCS+ key management (NIST-standardized, hash-based, no new mathematical assumptions) to replace ECDSA/ECIES at the DEK encryption layer. AES-256-GCM data encryption remains quantum-resistant. See `ARCHITECTURE.md` §Data Privacy Policy → Quantum Readiness.
...
## 9. Data Lifecycle & Privacy Considerations (2026-07-25)

### 9.1 Data Retention Policy

- **Active account**: Data is retained while the user has an account.
- **Deletion**: User can delete verified data at any time via "Delete Verified Data".
- **Re-activation**: If a deleted user returns, their data starts fresh (no recovery of encrypted data). The old encrypted data is unrecoverable without the original wallet-derived DEK.

### 9.2 Storage Strategy Per Verification State

| State | Storage | Rationale |
|-------|---------|-----------|
| Pre-verification | Plain text | User is actively filling profile; data needed for UI |
| Post-verification | Encrypted (AES-256-GCM) | Verification done → encrypt immediately |
| Verified hash | Store hashes of verified fields | Allows reverification without decrypting full data |

Flow: user fills profile → verifier confirms → data encrypted → plain text deleted → hash stored for comparison.

### 9.3 Premium Course Credentials & Restricted Regions

Premium course completion credentials (SBTs/NFTs) are **on-chain and public**. In a restricted region (region type 2) this exposes the holder as having taken a category B course.

**Mitigation options to evaluate:**
- **Soulbound tokens with encrypted metadata**: Token on-chain, metadata encrypted off-chain
- **Private credentials via Self.xyz ZK proofs**: Prove course completion without revealing course identity
- **Separate burner wallet for courses**: User claims credentials to a wallet not linked to their public identity
- **Delayed minting**: Credentials only minted when user explicitly requests, with clear warning

This must be addressed before launching premium courses in high-risk countries.

### 9.4 Privacy Policy Updates

When encryption is implemented, update the privacy policy to state:
- "Your verified personal data is stored encrypted and can only be decrypted by you (via your wallet) or the Pasos de Jesús team for verification purposes."
...
**Created:** 2026-06-29
**Status:** Pending — verified 2026-08-22: no `lib/encryption.ts`, no `encrypted_data`/`encrypted_dek`/`public_key` columns on `usuario`/`billetera_usuario`; sensitive data is still in plain text. All phases pending.
**Revisión 2026-09-25 (código):** se repitió la verificación: no existe `lib/encryption.ts` ni
ninguna columna `encrypted_data`/`encrypted_dek`/`public_key` en el esquema, y los datos
sensibles (nombres, WhatsApp, documentos) siguen en texto plano. **No cerrable:** todas las
fases siguen pendientes; es además dependencia de R-#162 Fase 2 y R-#164.
