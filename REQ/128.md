...
| **Distribution percentages (Standard)** | Premium payment distribution (55% pdJ, 10% vault, 10% reward, 10% missional, 2.5% UBI, 2.5% referrals, 5% churches) |
| **Distribution percentages (GD)** | Global Disciples course: 45% pdJ, 10% ClusterFunds, 10% vault, 10% reward, 10% missional, 2.5% UBI, 2.5% referrals, 5% churches (see §1.3) |
...
| Churches | 10% | 5% USDT, 5% SLEARN equivalent |
| **Total** | **100%** | |
...
| Destination | Percentage | Notes |
| :--- | :--- | :--- |
| pdJ Treasury | 45% | Operations & creators (10% less than standard) |
| **Cluster/Country Fund** | **10%** | Sent to #155 (ClusterFunds) for the user's cluster or country |
| Course Vault | 10% | 5% USDT, 5% SLEARN equivalent |
| Student Reward | 10% | Minted as SLEARN to student |
| Missional Courses | 10% | 5% USDT, 5% SLEARN equivalent |
| UBI | 2.5% | 1.25% USDT, 1.25% SLEARN equivalent |
...
**Why the difference?** For GD, 10% of the payment goes to the Cluster/Country Fund (#155) to support the cluster system. This 10% comes from the pdJ Treasury portion (55% → 45%).
...
| # | Task | Depends on |
|---|---|---|
| 2.1 | `POST /api/courses/premium/purchase` — call `SLEARN.receive()`, record enrollment, mint access SBT | 1.2, 1.3 |
...
| # | Task | Depends on |
|---|---|---|
| 3.1 | `SLEARN.receive()` with GD percentages (45% pdJ, 10% ClusterFunds) | #155 Phase 1 |
