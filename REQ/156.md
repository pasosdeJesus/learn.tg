# R-#156: Implement Donation System for Clusters/Countries
...
### 1.1 Entry Points: Ranking Pages

Donations are initiated from the **ranking pages** (R-#154), not from a standalone donation page. This makes discovery and action happen in the same place.
...
| 3   | Clúster Anónimo   | 🌍     | $120 | 480   | [Donar] |
```

- **[Donar]** en fila de país → destino pre-llenado: país (sin clúster específico). Fondos se reparten entre todos los clústeres de ese país (R-#155).
- **[Donar]** en fila de clúster → destino pre-llenado: clúster específico.

Sin dropdowns de selección de destino. El contexto ya viene del ranking.

### 1.2 User Journey

```mermaid
flowchart TB
    A[User browses ranking] --> B{Clicks Donar on...}
    B -->|Country row| C[Modal: country pre-filled]
    B -->|Cluster row| D[Modal: cluster pre-filled]
    C --> E[Enters amount + pdJ %]
    D --> E
    E --> F[Connects wallet]
    F --> G[Confirms donation]
    G --> H[Single transaction per crypto to ClusterFunds]
    H --> I[Backend verifies, splits, records in transaction table]
    I --> J[Ranking updated]
    J --> K[Receipt shown]
```
...
|------|-------------|-----------|
| **USDT** | Donate USDT to country or cluster | Amount input with USDT balance display |
| **SLEARN** | Donate SLEARN to country or cluster | Amount input with SLEARN balance display |
...
The same modal serves both country and cluster donations — only the destination label changes based on which [Donar] button was clicked.

### 2.1 Country Donation (from country row)

```
┌──────────────────────────────────────────┐
│  Donar a Sierra Leona               ✕   │
│                                          │
...
│                                          │
│  Resumen:                                │
│  → Sierra Leona: 8.50 USDT (85%)        │
│  → pdJ: 1.50 USDT (15%)                 │
│  Se reparte entre 3 clústeres           │
│                                          │
│              [Cancelar]  [Donar]         │
└──────────────────────────────────────────┘
```

### 2.2 Cluster Donation (from cluster row)

```
┌──────────────────────────────────────────┐
│  Donar a Clúster Esperanza          ✕   │
│                                          │
│  Tu saldo: 50.00 USDT | 120 SLEARN      │
│                                          │
│  USDT:  [____]              [Max] [✕]   │
...
│                                          │
│  Resumen:                                │
│  → Clúster Esperanza: 8.50 USDT (85%)  │
│  → pdJ: 1.50 USDT (15%)                 │
│                                          │
│              [Cancelar]  [Donar]         │
└──────────────────────────────────────────┘
...
| **Balance sufficient (CELO)** | User must have enough CELO for gas.  learn.tg provides a daily UBI of 0.2 CELO (reclaimable via the button in Guide 3 of the "Web3 & UBI" course). Users should claim this before donating. |
...
| No destination | "Missing destination. Please start from the ranking page." |
...
| Insufficient CELO | "You don't have enough CELO for gas (balance: X, estimated: Y). You can claim 0.2 CELO daily from the UBI button in Guide 3 of the Web3 & UBI course." |
...
### 9.1 Donation Buttons

Donation is initiated exclusively from the **ranking pages** (R-#154). No standalone donation page. The context pre-fills the destination.

| Page | Button | Destination |
|------|--------|-------------|
| Ranking — Country row | `[Donar]` | Country (split among all clusters) |
| Ranking — Cluster row | `[Donar]` | Specific cluster |

**Design principle:** Discovery and action in the same place. A donor browsing the ranking sees who needs funds and donates immediately.
...
- [ ] `[Donar]` button on every country row in ranking → opens modal with country pre-filled
- [ ] `[Donar]` button on every cluster row in ranking → opens modal with cluster pre-filled
