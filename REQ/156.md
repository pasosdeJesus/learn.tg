...
Every completed donation is also recorded in the existing `transaction` table:

```sql
-- One entry per crypto per donation (e.g., if user donates USDT + SLEARN → 2 rows)
INSERT INTO transaction (
    usuario_id, wallet, crypto, type, amount, date, hash,
    categoria, subcategoria, metadata
) VALUES (
    :usuario_id, :wallet, 'USDT', 'donation', :amount, NOW(), :hash,
...
- [ ] Every completed donation recorded in `transaction` table
