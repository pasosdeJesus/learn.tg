# SLEARN Frequently Asked Questions


***How can I earn SLEARN?***

Do the following before June 1, 2026:

* Verify your identity in [learn.tg](https://learn.tg) with self and  solve crosswords -> 1 Learning Point per crossword
* Donate 0.02 USDT or more on [sivel.xyz](https://sivel.xyz) -> 1 Learning Point per donation (first 100 donations only)
* Donate to courses on [learn.tg](https://learn.tg) -> points are proportional to the amount

On June 1, all your Learning Points turn into SLEARN (1:1) in your wallet.

***Can I send SLEARN to someone else? Can I cash it out?***

No, you cannot send SLEARN to anyone.

* Use SLEARN to pay for premium courses on learn.tg (that's what it's for).
* If you live in Sierra Leone **and** you've already paid for a premium course with SLEARN, in the phase 3 you will be able to exchange SLEARN for Leones (SLE) on [stable-sl.pdJ.app](https://stable-sl.pdJ.app).


***Is SLEARN a stablecoin?***

Kind of, but better: 1 SLEARN = at least 1 Leone (SLE).
If the Leone goes up, SLEARN goes up with it. If the Leone drops, SLEARN stays at 4.5 cents USD.

***Who can use SLEARN?***

Anyone with a verified learn.tg account.

* Outside Sierra Leone: only for paying courses.
* In Sierra Leone: same as above, plus you can turn SLEARN into Leones (after you've paid for at least one course).

***Where can I see how many SLEARN exist, reserves, etc?***

All live on the Transparency Dashboard: https://learn.tg/en/transparency

Right now it shows users, points, USDT/CELO given. After launch (June 1) it will also show SLEARN supply, burned tokens, and reserve balances.
