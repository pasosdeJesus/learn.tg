 ...
 *
 * V2 additions: processClusterContribution / processCountryContribution credit the
 * fund with the FULL amount (no fees, no cashback). Used when GD premium course
 * payments route their 10% to the cluster/country fund — fees and donor rewards
 * are already handled by SLEARN.processPayment, so the routed amount must arrive
 * intact. Also doubles as the migration entry point when upgrading contracts.
 *
 * Security: Uses OpenZeppelin's SafeERC20, ReentrancyGuard, Ownable, and Pausable.
 */
contract ClusterFundsV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    event ClusterContribution(address indexed clusterWallet, uint256 usdtAmount, uint256 slearnAmount);
    event CountryContribution(string countryCode, uint256 usdtAmount, uint256 slearnAmount);
    ...
    // ──── Course Payment Contributions ────

    /**
     * @notice Credit a cluster fund with the FULL routed amount (no fees, no cashback).
     * @dev Used when GD premium course payments route 10% to the cluster fund.
     *      Fees and donor rewards are handled by SLEARN.processPayment, so the
     *      routed amount must arrive intact. Also used to seed migrated balances.
     * @param txHash The transaction hash of the incoming transfer (replay protection).
     * @param clusterWallet The cluster's wallet address receiving the funds.
     * @param usdtAmount Exact USDT amount of this contribution.
     * @param slearnAmount Exact SLEARN amount of this contribution.
     */
    function processClusterContribution(
        bytes32 txHash,
        address clusterWallet,
        uint256 usdtAmount,
        uint256 slearnAmount
        ...
        emit ClusterContribution(clusterWallet, usdtAmount, slearnAmount);
    }

    /**
     * @notice Credit a country fund with the FULL routed amount (no fees, no cashback).
     * @dev Used when GD premium course payments route 10% to the country fund.
     *      Fees and donor rewards are handled by SLEARN.processPayment, so the
     *      routed amount must arrive intact. Also used to seed migrated balances.
     * @param txHash The transaction hash of the incoming transfer (replay protection).
     * @param countryCode ISO 3166-1 alpha-2 country code (e.g., "SL", "CO").
     ...
     * @param slearnAmount Exact SLEARN amount of this contribution.
     */
    function processCountryContribution(
        bytes32 txHash,
        string calldata countryCode,
        uint256 usdtAmount,
        uint256 slearnAmount
        ...
        countryFunds[countryCode].usdtBalance += usdtAmount;
        countryFunds[countryCode].slearnBalance += slearnAmount;

        emit CountryContribution(countryCode, usdtAmount, slearnAmount);
    }
