...
| `lockWallet()` | Clears the in-memory key (the sealed key and the password record stay) |
...
| `exportMnemonic(pin, storage?)` / `exportPrivateKey(pin, storage?)` | Password-protected export, no side effects on the session (a wallet imported from a private key has no phrase) |
...
| `getInAppWalletProvider({ rpcUrl?, requireUserVerification? })` | EIP-1193 provider when unlocked, `null` otherwise. `eth_sendTransaction` asks for a user-verified assertion (layer L1); a **known** destination is skipped within the 15-minute grace window, a **new** one always asks (R-#253); `requireUserVerification: false` disables that for tests |
| `isKnownDestination(wallet, to)` / `rememberDestination(wallet, to)` / `clearDestinations()` | Known-transfer destinations, per wallet (R-#253) |
...
The design decisions — the framework-agnostic boundary, what is stored, the secret
derivation (AES-256-GCM + PBKDF2), the **L0/L1/L2** unlock layers and the EIP-1193
provider — are in [ARCHITECTURE.md](ARCHITECTURE.md).
...
const { walletInfo } = await createWallet({ pin: 'learn-tg-e2e', storage })
...
`npm test` runs the unit suite in the `node` environment (47 tests): encryption
round-trip and wrong password, wallet lifecycle, the `isValidPassword` policy,
signature verification against `viem`'s `verifyMessage`, and the EIP-1193 provider
methods. The `IndexedDBStorage` test is skipped unless `fake-indexeddb` is installed.
