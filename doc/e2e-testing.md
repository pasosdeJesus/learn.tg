...
| `privacy-settings.spec.mjs` | R-#259: the two privacy switches of `/{lang}/settings` vs the public profile (`GET /api/user/<id>` without cookies) — turning publication off removes the credentials and turning it back on returns them, **and the switch is checked visually** (the thumb moves and the track changes colour; `data-state` alone changes even when the CSS is missing) |
