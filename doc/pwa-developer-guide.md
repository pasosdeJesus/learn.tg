...
| Offline submissions | `lib/offline-queue-db.ts` + `lib/hooks/useOfflineQueue.ts` | Generic queue (url + body), replayed on `online`; a server rejection (4xx) is exposed as `lastRejection` so the page tells the user instead of leaving the answer queued silently. The queued body carries `offlineSavedAt` (R-#242), so `/api/check-crossword` also leaves the result in `notifications` (`lib/offline-answer-notice.ts`) and the bell shows it even if the student already left the page. The drain takes a **cross-tab** lock (`navigator.locks`, `learn-tg-offline-drain`): `activeFlush` is module state, i.e. per tab, so with two tabs open the same answer was sent twice and the second one hit the contract's 24 h cooldown (found by `offline-two-courses.spec.mjs`, 2026-09-25). A **401/403 does not consume attempts either** (a 4xx other than that one does, and after `MAX_ATTEMPTS` = 5 the answer is dropped): the item stays in the queue, the drain stops and the student is told to sign in again (`needsSignIn`, option A of https://github.com/pasosdeJesus/learn.tg/issues/234 §4.9) |
