...
|---|----------|----------|---------|-------------|
| 1 | Course name | ✅ | `"Global Disciples"` | Display title |
| 2 | Language | ✅ | `en` or `es` | Course language — determines guide template and DB `idioma` |
| 3 | URL prefix | ✅ | `"global-disciples"` | Lowercase, hyphens — used in URLs and file paths |
| 4 | Number of guides | ✅ | `4` | How many guide `.md` templates to generate |
| 5 | USDT per guide | ✅ | `"1.0"` | Scholarship USDT amount per completed guide |
| 6 | SLEARN per guide | ✅ | `"1.0"` | Scholarship SLEARN amount per completed guide |
| 7 | Icon path | ❌ | `"source/my-icon.svg"` | Path under `public/img/credential/`. If omitted, a generic course icon is generated |
| 8 | Course ID | ❌ | `5` | Explicit DB ID. If omitted, auto-generated by the sequence |

### Icon default

When no icon path is provided, `credentials:register-type` generates a default credential icon from the course name text. This is suitable for initial setup — replace it later with a custom 512×512 SVG.
...
# English course, auto-generated ID, no icon:
./scripts/create-course.sh "Global Disciples" en "global-disciples" 4 "1.0" "1.0"

# Spanish course with icon, explicit ID:
...
- Kysely migration at `db/migrations/<timestamp>_create_course_5.ts` with course + guide INSERTs
...
### 1.1 Review and run the generated migration

The script creates a Kysely migration with proper `up()` and `down()` functions.

```bash
# Review the migration first
cat db/migrations/<timestamp>_create_course_5.ts

# Run all pending migrations
bin/m db:migrate
```

The migration inserts into `cor1440_gen_proyectofinanciero` (course) and `cor1440_gen_actividadpf` (guides) with auto-calculated IDs.
...
> **Why migrations?** Unlike raw SQL files, Kysely migrations are versioned, trackable, reversible, and become part of the project history. Every environment runs the same sequence deterministically.

> **Bilingual courses**: Run the script twice — once with `en` and once with `es`. Each invocation creates a separate course record with its own language and prefix.
