...
vi.mock('fs/promises', async (importOriginal) => {
  ...
  return {
    ...actual,
    readFile: vi.fn(() =>
      Promise.resolve('# Crossword Source\n\nTexto con blanks'),
    ),
  }
})
...
vi.mock('@/lib/remarkFillInTheBlank.mjs', () => ({
  remarkFillInTheBlank: () => () => {
    ;(global as any).fillInTheBlank = [
...
vi.mock('kysely', () => {
  class FakeKysely {
    ...
    executeTakeFirst() {
      return Promise.resolve({ id: 1, token: 'tok123' })
    }
    updateTable() {
      return {
...
describe('API /api/crossword', () => {
  ...
  it('retorna grid y placements con parámetros completos y wallet', async () => {
    const url = new URL(
      'http://localhost:3000/api/crossword?courseId=c1&lang=en&prefix=a-relationship-with-Jesus&guide=guide1&walletAddress=0xabc&token=tok123',
    )
  ...
  it('retorna mensaje si falta walletAddress pero sigue con 200', async () => {
    const url = new URL(
      'http://localhost:3000/api/crossword?courseId=c1&lang=en&prefix=a-relationship-with-Jesus&guide=guide1',
    )
    const request = new NextRequest(url)
    const response = await GET(request)
    const data = await response.json()
