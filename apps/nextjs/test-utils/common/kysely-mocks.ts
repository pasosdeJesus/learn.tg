...
export function createMockKysely(options: {
  ...
  execute?: any
  sqlExecute?: any
  /** Custom module path to mock for Kysely configuration (default: @/.config/kysely.config) */
  configPath?: string
  /** Whether to skip mocking the config module (default: false) */
  skipConfigMock?: boolean
} = {}) {
  const opts = options
  ...
  if (opts.executeTakeFirst !== undefined) {
  ...
  return {
    ...
    setupMocks() {
      ...
      // Mock config module only if not skipped
      const skipConfigMock = opts.skipConfigMock || false
      if (!skipConfigMock) {
