 ...
 *
 * Decisión de privacidad (R-#259 §3.3 / R-#256 §3.6b): un curso `contenido_sensible`
 * solo se descarga con el interruptor de publicar contenido sensible encendido, y
 ...
 * borrarla y al apagar el interruptor de contenido sensible (R-#256 §3.5/§3.6b).
 */
export async function deleteDownloadedCourses(
  filter: { premium?: boolean; sensitive?: boolean; wallet?: string } = {},
): Promise<string[]> {
  ...
  for (const course of courses) {
    ...
    const matchesSensitive = filter.sensitive === undefined || course.contenidoSensible === filter.sensitive
    const matchesWallet = filter.wallet === undefined
      || (course.wallet || '').toLowerCase() === filter.wallet.toLowerCase()
    if (matchesPremium && matchesSensitive && matchesWallet) {
 ...
 * sensible (categoría B; el interruptor de R-#259 vive en la sesión). Se llama al
 * desconectar, al borrar la billetera y al apagar el interruptor. Las respuestas
 * en cola (store `pending`) **no** se tocan: R-#240 §4b item 9.
 */
export async function clearRestrictedCourseCopies(): Promise<string[]> {
  const courses = await listDownloadedCourses()
  const keys = courses
    .filter((course) => course.isPremium || course.contenidoSensible)
