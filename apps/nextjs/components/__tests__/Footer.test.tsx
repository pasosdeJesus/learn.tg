...
describe('Footer', () => {
  it('shows Spanish Telegram link when lang="es"', () => {
    ...
    expect(screen.getByRole('link', { name: "Grupo en Telegram" })).toHaveAttribute('href', 'https://t.me/learn_t_g');
  });

  it('shows English Telegram link when lang="en"', () => {
    render(<Footer lang="en" />);
    expect(screen.getByText("Telegram group")).toBeInTheDocument();
    expect(screen.getByRole('link', { name: "Telegram group" })).toHaveAttribute('href', 'https://t.me/learn_t_g');
  });
