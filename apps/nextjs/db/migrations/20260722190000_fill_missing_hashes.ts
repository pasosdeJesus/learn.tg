// Migration: Fill missing hashes and dates from blockchain history CSV.
//
// Reads doc/hashes-2026-07-23.csv and updates transaction entries that
// have NULL hash with the real blockchain hash and timestamp.
// Matches by (usuario_id, course_id, guide_id) from metadata.
...
export async function up(_db: Kysely<any>): Promise<void> {
  ...
  // Connect with pg directly to avoid Kysely template literal issues
  const pool = new Pool({
  ...
  const { rows } = await pool.query(
    `SELECT id, usuario_id, metadata FROM transaction
     WHERE type = 'scholarship' AND crypto = 'usdt' AND hash IS NULL`
  )
  ...
  for (const r of rows) {
    ...
    await pool.query(
      `UPDATE transaction SET hash = $1, date = $2::timestamptz,
        metadata = metadata || '{"source":"blockchain"}'::jsonb,
        updated_at = NOW()
       WHERE id = $3`,
  ...
  await pool.end()
  console.log('Filled hashes and dates for ' + filled + ' entries')
}
